# Modified source from https://coldcard.com/docs/verifying-dice-roll-math
...
def get_hash_from_rolls(r):
    return sha256(r.encode()).digest()

def get_wordindex_from_hash(r, h):
...
def format_wordindex_to_list(r, h):
    return ['%d: %s' % (n+1, wl[i]) for n, i in enumerate(get_wordindex_from_hash(r, h))]

if __name__ == "__main__":
    r = input().strip()

    h = get_hash_from_rolls(r)
    print(h.hex())
    print()

    # Print index number and each word (24)
    print('\n'.join('%4d: %s' % (n+1, wl[i]) for n, i in enumerate(get_mneumonics_from_hash(h))))